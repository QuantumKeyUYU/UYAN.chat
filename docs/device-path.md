...
- **Источник правды** — `useAppStore().deviceId`. Все запросы к защищённым API добавляют `x-device-id` из стора.
...
- `useAppStore().setDeviceId` по-прежнему источник правды. После успешной привязки API возвращают основной `deviceId`, и стор синхронизирует cookie/`localStorage`.
