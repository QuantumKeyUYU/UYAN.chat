...
export default function GardenPage() {
  ...
  const downloadAsImage = async () => {
    ...
    try {
      ...
      link.download = `uyan-archive-${shareLight.id}.png`;
      link.href = dataUrl;
      link.click();
    } catch (error) {
  ...
  return (
    ...
    >
      <div className="space-y-2">
        ...
        <p className="text-text-secondary">Здесь живут слова поддержки, которые хочется сохранить рядом.</p>
      </div>
        ...
        <Card className="text-center">
          <div className="space-y-4">
            ...
            <p className="text-lg text-text-secondary">
              Пока архив пуст. Сохраняй отклики из раздела «Мои отклики», чтобы возвращаться к ним в нужный момент. С ключом из
              настроек архив переедет с тобой на любое устройство.
            </p>
          </div>
        </Card>
      ) : (
        <div className="grid gap-6 md:grid-cols-2">
          {lights.map((light) => (
            <Card key={light.id} className="space-y-4">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-uyan-darkness">ты поделился мыслью</p>
                <p className="mt-2 rounded-xl bg-bg-tertiary/50 p-4 text-sm text-text-secondary">{light.originalMessage}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-uyan-light">получил отклик</p>
      ...
      <Modal open={shareOpen} onClose={savingImage ? () => {} : closeShare} title="Поделиться откликом">
        {shareLight ? (
          ...
          <p className="text-center text-text-secondary">Выбери отклик, чтобы поделиться теплом.</p>
        )}
      </Modal>
    </motion.div>
  );
