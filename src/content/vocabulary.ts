...
const presets: Record<VocabularyPreset, Vocabulary> = {
  core: {
    ...
    homeTileAnswersBody:
      'Сохраняй слова поддержки, которые ты получаешь. К ним можно возвращаться в тяжёлые дни.',
    ...
    homeHeroSubtitle:
      'Здесь можно честно написать, как тебе на самом деле, и получить тёплый ответ.\nИли увидеть чью-то мысль и поддержать её несколькими фразами.\nБез имён, лайков и гонки за вниманием — только анонимные письма поддержки.',
    ...
    flow: {
      ...
      saveDescription:
        'Письма поддержки остаются в разделе «Ответы». К ним можно возвращаться, когда тяжело.',
    },
  },
  spark: {
    message: 'искра',
